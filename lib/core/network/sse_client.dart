...
class SseClient {
  SseClient({required this.baseUrl, ChatTransport? transport})
  ...
  Stream<ChatEvent> chatStream(ChatRequest request) async* {
    ...
    try {
      bytes = await _transport.open(_chatUri, jsonEncode(request.toJson()));
...
/// App-wide SSE client; rebuilt when the configured base URL changes.
final sseClientProvider = Provider<SseClient>(
  (ref) => SseClient(baseUrl: ref.watch(appConfigProvider).baseUrl),
);
